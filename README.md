...
## Melhorias e organização aplicadas no frontend

- Separação de responsabilidades entre **controller** e **services** (comunicação HTTP centralizada).
- CRUD dividido em métodos explícitos `criarContato` e `atualizarContato`.
- Centralização da URL base da API em `configValue`.
- Filtro `name` para padronização de nomes exibidos.
- Máscara de telefone com diretiva `telefoneMask`.
